...
fn test_connection_closed_regex() {
    ...
    let line2 = "Connection closed by authenticating user admin 198.51.100.9 port 59900 [preauth]";
    assert_eq!(
        re.captures(line2).unwrap().get(1).unwrap().as_str(),
        "198.51.100.9"
    );
...
fn test_disconnecting_regex() {
    let re =
        Regex::new(r"Disconnecting (?:invalid user|authenticating user) \S+ (\d+\.\d+\.\d+\.\d+)")
            .unwrap();
...
fn test_max_auth_attempts_regex() {
    let re =
        Regex::new(r"maximum authentication attempts exceeded for .* from (\d+\.\d+\.\d+\.\d+)")
            .unwrap();
