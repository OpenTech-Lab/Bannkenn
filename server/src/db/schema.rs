...
pub(crate) async fn migrate(pool: &SqlitePool) -> anyhow::Result<()> {
    ...
    let _ = sqlx::query("ALTER TABLE behavior_events ADD COLUMN parent_command_line TEXT")
        .execute(pool)
        .await;
    let _ = sqlx::query("ALTER TABLE behavior_events ADD COLUMN parent_pid INTEGER")
        .execute(pool)
        .await;
    let _ = sqlx::query("ALTER TABLE behavior_events ADD COLUMN uid INTEGER")
        .execute(pool)
        .await;
    let _ = sqlx::query("ALTER TABLE behavior_events ADD COLUMN gid INTEGER")
        .execute(pool)
        .await;
    let _ = sqlx::query("ALTER TABLE behavior_events ADD COLUMN service_unit TEXT")
        .execute(pool)
        .await;
    let _ = sqlx::query("ALTER TABLE behavior_events ADD COLUMN first_seen_at TEXT")
        .execute(pool)
        .await;
    let _ = sqlx::query("ALTER TABLE behavior_events ADD COLUMN trust_class TEXT")
        .execute(pool)
        .await;
    let _ = sqlx::query("ALTER TABLE behavior_events ADD COLUMN trust_policy_name TEXT")
        .execute(pool)
        .await;
    ...
    let _ = sqlx::query("ALTER TABLE behavior_events ADD COLUMN container_runtime TEXT")
        .execute(pool)
        .await;
    let _ = sqlx::query("ALTER TABLE behavior_events ADD COLUMN container_id TEXT")
        .execute(pool)
        .await;
